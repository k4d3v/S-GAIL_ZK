    ...
    for p in range(paths_per_collect):
        ...
        for i in range(max_step_limit):
            ...
            if np.argmax(encode) == 0:
                if i + 1 >= min_step_limit and np.sum(abs(obs[-3:])) <= 0.018 and abs(action[0][0])< 5e-4 and abs(action[0][0])< 5e-4:
                    goal_flag = 1
                    count_goalagent+=1

            elif np.argmax(encode) == 1:
                if i + 1 >= min_step_limit and np.sum(abs(obs[-3:])) >= 0.40 and abs(action[0][0])< 5e-4 and abs(action[0][0])< 5e-4:
                    goal_flag = 1
                    count_goalagent+=1
