...
parser.add_argument("--paths_per_collect", type=int, default=10)  # 20
...
parser.add_argument("--n_iter", type=int, default=7)  # 3000
...
parser.add_argument("--inner_loop", type=int, default=5)  # 10
...
class TRPOAgent(object):
    ...
    def __init__(self, sess, state_dim, encode_dim, action_dim, actions_output_dim, filepath):
        ...
        :param sess: tf session; to be replaced by pyTorch
        :param state_dim:
        :param encode_dim:
        :param action_dim:
        :param actions_output_dim: TODO: What is the difference between action and actions_output?
        :param filepath: Where the learnt models should be stored
        """
        #
        # Initialize Network
        #
        ...
        # Create tf objects for storing states, actions, policy etc.
        self.state = state = tf.placeholder(dtype, shape=[None, state_dim])
        # 4時刻前までの環境を入力(10次元ベクトル)
    ...
    def create_generator(self, state, encodes, action_dim):
        """
        Creates Generator net
        :param state: State dim
        :param encodes: Dim of task var
        :param action_dim: Dim of action. TODO: Remove hardcoded part
    ...
    def create_discriminator(self, state, action, noise, encode, policy):
        """
        Creates Discriminator net.
        :param state:
        :param action:
        :param noise: Gaussian noise
        :param encode: Task var dim.
        :param policy:
        :return: New model
        """
        # TODO: tf to pyTorch
    ...
    def learn(self, state_expert, action_expert, action_expert_ori, s_max, s_min, a_max, a_min, new_dir_path):
        """
        Impl. of S-GAIL learning procedure.
        In each iter. generates trajs from G and updates G (via TRPO; based on value fun extimate)
        and D (Adam optimizer; Based on classification error).
        :param state_expert: Normed states of expert
        :param action_expert: Normed actions of expert
        :param action_expert_ori: Original expert actions
        :param s_max:
        :param s_min:
        :param a_max:
        :param a_min:
        :param new_dir_path: Path to dir where newly learnt models should be stored
        :return: Counter after last iteration
        """
        #
        # Information Writing
        #
        ...
        numeptotal = 0
        start_epoch = 0
        progress = 0  # Counter for iterations
        ...
        for i in range(start_epoch, start_epoch + config.n_iter):
            ...
            progress += 1  # Increment current iteration counter
            ...
            for iiter in range(self.config.inner_loop):
                ...
                if iiter == self.config.inner_loop - 1:
                    ...
                    for path in paths_new:
                        ...
                        file_path_world = new_dir_path + \
                                          "/Generator_trajectories_resample/iter_%d_world_%d" % (i, path_idx)
